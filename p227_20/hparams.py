...
hparams = tf.contrib.training.HParams(
	...
	cleaners="english_cleaners",#'english_cleaners',
	...
	trim_top_db = 63, #Trimming db difference from reference db (smaller==harder trim.)
	...
	upsample_type = '1D', #Type of the upsampling deconvolution. Can be ('1D' or '2D', 'Resize', 'SubPixel' or simple 'NearestNeighbor').
	...
	tacotron_test_size = None, #% of data to keep as test data, if None, tacotron_test_batches must be not None. (5% is enough to have a good idea about overfit)
	tacotron_test_batches = 1, #number of test batches.
	...
	tacotron_final_learning_rate = 1e-6, #minimal learning rate
	...
	tacotron_teacher_forcing_decay_alpha = 0., #teacher forcing ratio decay rate. Defines the final tfr as a ratio of initial tfr. Relevant if mode='scheduled'
