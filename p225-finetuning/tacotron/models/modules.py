...
class ReferenceEncoder:
	def __init__(self,is_training,hparams,scope):
		self.is_training = is_training
		self.scope = "reference_encoder" if scope is None else scope
		self.filters = hparams.ref_enc_filters
		self.kernel_size = hparams.ref_enc_size
		self.stride = hparams.ref_enc_stride
		self.pad = hparams.ref_enc_pad
		self.activation = tf.nn.relu
		self.gru = GRUCell(hparams.reference_depth)

	def __call__(self,inputs):
		with tf.variable_scope(self.scope):
			x = inputs
			for i,filter in enumerate(self.filters):
				x = conv2d(x,self.kernel_size,self.stride,filter,self.activation,self.is_training,self.drop_rate,
						   "conv_layer_{}_".format(i+1)+self.scope)
		shapes = shape_list(x)
		ref_outputs = tf.reshape(x,shapes[:-2]+[shapes[2]*shapes[3]])

		#RNN
		encoder_outputs,encoder_state = tf.nn.dynamic_rnn(self.gru,ref_outputs,dtype=tf.float32)

		return tf.expand_dims(encoder_outputs[: -1:],axis=1)
...
class STL:
	def __init__(self,is_training,hparams,scope):
		...
		self.embed = tf.get_variable(
			"style_token_embedding",[hparams.token_num,hparams.token_emb_size // hparams.num_heads],
			dtype = tf.float32,initializer = tf.truncated_normal_initializer(stddev=0.5)
		)
		self.num_heads = hparams.num_heads
		self.num_units = hparams.multihead_attn_num_unit

	def __call__(self, inputs,scale = None):
		with tf.variable_scope(self.scope):
			keys = F.tanh(self.embed).unsqueeze(0).expand(N,-1,-1)
			if scale is None:
				scale_index = scale.keys()[0]
				scale_value = scale.values()[0]
				scale_array = np.zeros(c.E // c.num_heads)
				scale_array[:] = scale_value
				style_embed = keys[0][scale_index]*scale_array
				style_embed = np.tile(style_embed,(N,1))
			else:
				style_attention = MultiHeadAttention(inputs,keys,num_heads = self.num_units)
				style_embeddings = style_attention.multi_head_attention()
		return style_embed

class MultiHeadAttention:
	def __init__(self,query,key,num_heads=8,attention_type="mlp_attention",num_units = None,normalize = True):
		self.query = query
		self.key = key
	...
	def multi_head_attention(self):
		with tf.variable_scope("Multihead_attention"):
			Q = tf.layers.dense(self.query,self.num_units,activation=tf.nn.relu)
			K = tf.layers.dense(self.key,self.num_units,activation = tf.nn.relu)
			V = tf.layers.dense(self.key,self.num_units,activation = tf.nn.relu)
			#split and concate
...
def conv2d(inputs, kernel_size, stride,channels, activation, is_training, drop_rate, scope):
	#assert bnorm in ('before', 'after')
