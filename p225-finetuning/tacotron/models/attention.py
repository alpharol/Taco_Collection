...
class LocationSensitiveAttention(BahdanauAttention):
  ...
  577–585.
	...
	def __init__(self,
		...
		self.attention_win_size = tf.convert_to_tensor(19, dtype=tf.int32)
		self.constraint_type = "window"
		self.forward_attention = hparams.forward_attention
	...
	def __call__(self, query, state, prev_max_attentions):
		...
		if self.forward_attention:
			offset_alpha = tf.concat([tf.zeros((self.batch_size,1)),previous_alignments[:,:-1]],axis=1)
			alpha = (previous_alignments + offset_alpha)*alignments
			alpha = tf.clip_by_value(alpha,1e-8,1)
			alignments = alpha / tf.expand_dims(tf.reduce_sum(alpha,[1]),1)
