...
class TacotronDecoderCell(RNNCell):
	...
	* : This is typically taking a vanilla LSTM, wrapping it using tensorflow's attention wrapper,
	...
	def __init__(self, prenet, attention_mechanism, rnn_cell, frame_projection, stop_projection,forword_attention = True):
		...
		self.forward_attention = forword_attention
	...
	def zero_state(self, batch_size, dtype):
		...
		with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
			...
			initial_alpha = tf.zeros((batch_size,self._attention_mechanism.alignments_size))
			ones_column = tf.ones((batch_size,1))
			initial_alpha = tf.concat([ones_column,initial_alpha[:,1:]],axis=1)
...
class TacotronGSTCell:
	def __init__(self,referenceencoder,stl):
