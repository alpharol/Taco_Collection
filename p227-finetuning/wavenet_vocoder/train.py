...
def eval_step(sess, global_step, model, plot_dir, wav_dir, summary_writer, hparams, model_name):
	...
	#Save Audio
	wavfile_write(pred_wav_path, hparams.sample_rate, y_hat)
	wavfile_write(target_wav_path, hparams.sample_rate, y_target)
...
def save_log(sess, global_step, model, plot_dir, wav_dir, hparams):
	...
	idx = 0
	y_hat, y, length = sess.run([model.tower_y_hat_log[idx],
		model.tower_y_log[idx],
		model.input_lengths[0][idx]])
